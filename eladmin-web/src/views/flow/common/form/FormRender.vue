<template>
  ...
  <el-form ref="form" class="process-form" label-position="top" :rules="rules" :model="_value">
    <el-form-item v-for="(item, index) in forms" v-if="item.name !== 'SpanLayout' && item.name !== 'Description'" :key="item.name + index" :prop="item.id" :label="item.title">
      <form-design-render :ref="`sub-item_${item.id}`" v-model="_value[item.id]" mode="PC" :config="item" />
    </el-form-item>
    <form-design-render v-else ref="span-layout" v-model="_value" mode="PC" :config="item" />
  </el-form>
</template>
